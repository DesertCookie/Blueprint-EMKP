...
public class BytecodeHelper {
	 ...
	 * @return A {@link Pair} with the left value being the combined ConstPool and the right value a HashMap with the keys being the original indices
	 * and their respective values the new indices
	 */
	...
	public static byte[] updateConstPoolRefs( byte[] code,HashMap<Integer,Integer> updatedIndices ) {
		for( int i = 0;i < code.length;i++ ) {
			...
			} else if(isTwoByteConstPoolOpcode( code[ i ] & 0xFF )) {
	...
	/**
	 * Injects the source code into the target code at the specified index. The returned bytecode should be without errors: this method automagically
	 * updates the {@link ConstPool} and references to it in the code.
	...
	public static Bytecode inject( MethodInfo source,MethodInfo target,int index ) {
		...
		Bytecode inject = new Bytecode(
				updatedConstPool.left,
