...
import io.github.wordandahalf.blueprint.transformers.method.*;
...
public class BlueprintAnnotationParser {
	...
	/**
	 * Scans the provided method for Blueprint-related annotations. The class declaration must be decorated with a {@link Blueprint} annotation for
	 * any other annotations to be processed.
	...
	public static List<ClassTransformer> parse( Class<?> clazz ) {
		...
		for( Method m : clazz.getDeclaredMethods() ) {
			...
			for( Annotation annotation : m.getAnnotations() ) {
				...
				} else if(annotation instanceof Overwrite) {
